...
class SelectThread(object):
    ...
    def remove_socket(self, s):
        ...
        with self._lock:
            self._file_descriptors = self._file_descriptors.difference([
                                                                       s.fileno()])
            new_file_descriptor_to_callback = self._file_descriptor_to_callback.copy()
    ...
    def _loop_forever(self):
        while shutdown and not shutdown.shutting_down():
            ...
            try:
                ...
                print "Error in ssl, emitting from ./google-cloud-sdk/platform/google_appengine/google/appengine/tools/devappserver2/wsgi_server.py"
...
class _SingleAddressWsgiServer(wsgiserver.CherryPyWSGIServer):
    ...
    def start(self):
        ...
        for res in info:
            ...
            break
        if not self.socket:
            raise BindError('Unable to bind %s:%s' %
                            self.bind_addr, socket_error)
...
class WsgiHostCheck(object):
    ...
    def __init__(self, whitelisted_hosts, app):
        ...
        for host in whitelisted_hosts:
            if host.startswith('**.'):
                self.whitelisted_deep_wildcard_hosts.add(
                    self._get_base_host_name(host))
            elif host.startswith('*.'):
                self.whitelisted_wildcard_hosts.add(
                    self._get_base_host_name(host))
            else:
...
class WsgiServer(object):
    ...
    def __init__(self, host, app, ssl_certificate_paths=None, ssl_port=None):
        ...
        if ((ssl_certificate_paths and not ssl_port) or
            ...
            raise ValueError(
                'Must specify both ssl_certificate_paths and ssl_port')
        self.bind_addr = host
        self._app = app
        self._servers = []
    ...
    def start(self):
        ...
        if port != 0:
            ...
            for _ in range(_PORT_0_RETRIES):
                ...
                raise BindError(
                    'Unable to find a consistent port for %s' % host)
